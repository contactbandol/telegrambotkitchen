...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ADD_"):
        ...
        await query.edit_message_text(
            ...
            reply_markup=build_menu_keyboard(order.get("page", 0)),
            parse_mode="Markdown"
        )
        return
    ...
    if data.startswith("STATUS_"):
        ...
        if status == "COOKING":
            ...
            new_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Готово", callback_data=f"STATUS_READY_{order_id_str}")]])
...
def daily_report():
    ...
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(application.bot.send_message(chat_id=DIRECTION_CHAT_ID, text=text, parse_mode="Markdown"))
    finally:
        loop.close()
    return "ok"
