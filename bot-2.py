...
ADMIN_ID = int(os.environ["ADMIN_ID"])  # ton Telegram user ID
...
# ─────────────────────────────────────────────
#  STATS — log persistant des commandes
# ─────────────────────────────────────────────
def load_stats():
...
def record_order(items):
    """Enregistre les plats d'une commande dans stats.json"""
    stats = load_stats()
    month_key = datetime.now().strftime("%Y-%m")  # ex: "2025-04"
...
def build_rapport(month_key):
    """Génère le texte du rapport pour un mois donné (format YYYY-MM)"""
    stats = load_stats()
    data = stats.get(month_key, {})

    year, month = map(int, month_key.split("-"))
    month_name = calendar.month_name[month]  # January, February...
    # Noms ukrainiens des mois
    UA_MONTHS = {
        1: "Січень", 2: "Лютий", 3: "Березень", 4: "Квітень",
        5: "Травень", 6: "Червень", 7: "Липень", 8: "Серпень",
        9: "Вересень", 10: "Жовтень", 11: "Листопад", 12: "Грудень",
    }
    month_ua = UA_MONTHS.get(month, month_name)
...
# ─────────────────────────────────────────────
#  TABLES — chargées depuis tables.json
# ─────────────────────────────────────────────
def load_tables():
...
# ─────────────────────────────────────────────
#  ÉTAT DES COMMANDES EN MÉMOIRE
# ─────────────────────────────────────────────
orders = {}
# order_counter pour numéroter les commandes
order_counter = {"n": 0}
...
def build_cart_keyboard():
    """Affiche le panier avec option de supprimer chaque plat"""
    rows = []
    rows.append([InlineKeyboardButton("📂 Добавить ещё", callback_data="BACK_CAT")])
    rows.append([InlineKeyboardButton("✅ Отправить заказ", callback_data="SEND")])
    rows.append([InlineKeyboardButton("❌ Отмена", callback_data="CANCEL")])
    return InlineKeyboardMarkup(rows)
...
def build_remove_keyboard(items_in_cart):
    """Keyboard pour supprimer des plats du panier"""
    menu = load_menu()
    # Construire un lookup id→name
    item_lookup = {}
    for cat in menu["categories"]:
        for item in cat["items"]:
    ...
    rows = []
    # On liste les plats uniques avec un bouton supprimer
    seen = []
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Привет! Бот bandôl готов к работе.\n\nНажми /new чтобы создать заказ."
    )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("CAT_"):
        ...
        await query.edit_message_text(
            ...
            reply_markup=build_items_keyboard(cat_id),
            parse_mode="Markdown",
        )
        return
    ...
    if data == "BACK_CAT":
        ...
        cart_text = format_cart(table_label, order["items"])
    ...
    if data.startswith("REMOVE_"):
        ...
        if item_id in order["items"]:
            order["items"].remove(item_id)  # retire une seule occurrence
    ...
    if data == "SEND":
        ...
        await query.edit_message_text(
            ...
            f"{format_cart(table_label, items)}",
            parse_mode="Markdown",
        )
        orders.pop(user_id, None)
        return
    ...
    if data.startswith("STATUS_"):
        parts = data.split("_")
        # FORMAT: STATUS_COOKING_1 ou STATUS_READY_1
        status = parts[1]
        ...
        if status == "COOKING":
            ...
            new_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton(f"✅ ГОТОВО", callback_data=f"STATUS_READY_{order_id}")]
            ])
        elif status == "READY":
            new_text = query.message.text + f"\n\n✅ *ГОТОВО* — {now}"
            new_keyboard = InlineKeyboardMarkup([])
