...
def build_cart_keyboard(comment=None):
    comment_label = "📝 " + comment[:20] + "..." if comment and len(comment) > 20 else ("📝 " + comment if comment else "📝 Додати коментар")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Додати ще", callback_data="PAGE_0")],
        [InlineKeyboardButton("Прибрати страву", callback_data="REMOVE_LIST")],
        [InlineKeyboardButton(comment_label, callback_data="ADD_COMMENT")],
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cart_text = format_cart(order.get("table_label", "?"), order.get("items", []))
    await update.message.reply_text(
        cart_text + "\n\n📝 _" + comment + "_\n\nПеревір замовлення:",
        reply_markup=build_cart_keyboard(comment),
        parse_mode="Markdown"
    )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "SHOW_CART":
        ...
        if comment:
            text += "\n\n📝 _" + comment + "_"
        await query.edit_message_text(text + "\n\nПеревір замовлення:", reply_markup=build_cart_keyboard(comment), parse_mode="Markdown")
        return
    ...
    if data == "ADD_COMMENT":
        ...
        await query.edit_message_text(
            cart_text + "\n\n📝 Напиши коментар (алергія, побажання тощо):\n_Просто надішли текст_",
            parse_mode="Markdown"
        )
        return
    ...
    if data == "SEND":
        ...
        confirm_text = "Замовлення #" + str(order_id) + " відправлено.\n\n" + format_cart(table_label, items)
        if comment:
            confirm_text += "\n\n📝 _" + comment + "_"
    ...
    if data.startswith("STATUS_PARTIAL_"):
        ...
        if not items:
            ...
            return
        already_served = order_served.get(oid, [])
        remaining = [i for i in items if i not in already_served]
        if not remaining:
            await query.answer("Всі страви вже подані", show_alert=True)
            return
        partial_sel[order_id_str] = set()
        await query.edit_message_reply_markup(reply_markup=build_partial_keyboard(order_id_str, remaining, set()))
        return
    ...
    if data.startswith("PTOGGLE_"):
        ...
        already_served = order_served.get(oid, [])
        remaining = [i for i in items if i not in already_served]
        await query.edit_message_reply_markup(reply_markup=build_partial_keyboard(order_id_str, remaining, partial_sel[order_id_str]))
        return
    ...
    if data.startswith("PCANCEL_"):
        ...
        kb = build_after_cooking_keyboard(oid or order_id_str) if "Готується" in msg_text else build_kitchen_keyboard(oid or order_id_str)
    ...
    if data.startswith("PCONFIRM_"):
        ...
        if not items and query.message and query.message.text:
            items = parse_items_from_message(query.message.text)
        already_served = order_served.get(order_id_int, [])
        remaining_items = [i for i in items if i not in already_served]
        ready_items = [i for i in remaining_items if i in selected]
        pending_items = [i for i in remaining_items if i not in selected]
        if order_id_int not in order_served:
            order_served[order_id_int] = []
        order_served[order_id_int].extend(ready_items)
        now = datetime.now().strftime("%H:%M")
        ...
        msg_text = query.message.text or ""
        kb = build_after_cooking_keyboard(order_id_int or order_id_str) if "Готується" in msg_text else build_kitchen_keyboard(order_id_int or order_id_str)
    ...
    if data.startswith("STATUS_"):
        ...
        if status == "COOKING":
            ...
            if order_id_int and order_id_int in order_waiter:
                ...
                order_served.pop(order_id_int, None)
        return
