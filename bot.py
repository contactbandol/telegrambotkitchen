...
order_counter = {"n": 0}
order_waiter = {}
order_items = {}       # items par order_id pour partiel
partial_sel = {}       # sélection en cours pour partiel
...
def format_cart(table_label, items):
    ...
    for item_id, count in counts.items():
        ...
        lines.append("• " + name + " ×" + str(count) if count > 1 else "• " + name)
    return "\n".join(lines)
...
def build_kitchen_keyboard(order_id):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("Готується", callback_data="STATUS_COOKING_" + str(order_id)),
            InlineKeyboardButton("Готово", callback_data="STATUS_READY_" + str(order_id)),
        ],
        [InlineKeyboardButton("🍽 Частково готово", callback_data="STATUS_PARTIAL_" + str(order_id))],
    ])
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("PTOGGLE_"):
        rest = data[8:]
        underscore_idx = rest.index("_")
        order_id_str = rest[:underscore_idx]
        item_id = rest[underscore_idx+1:]
    ...
    if data.startswith("PCANCEL_"):
        ...
        try:
            ...
            oid = None
        await query.edit_message_reply_markup(reply_markup=build_kitchen_keyboard(oid or order_id_str))
        return
    ...
    if data.startswith("PCONFIRM_"):
        ...
        await query.edit_message_text(updated_text, reply_markup=build_kitchen_keyboard(order_id_int or order_id_str), parse_mode="Markdown")
        return
    ...
    if data.startswith("STATUS_"):
        ...
        if status == "COOKING":
            ...
            await query.edit_message_text(new_text, reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Готово", callback_data="STATUS_READY_" + order_id_str)],
                [InlineKeyboardButton("🍽 Частково готово", callback_data="STATUS_PARTIAL_" + order_id_str)],
            ]), parse_mode="Markdown")
            if order_id_int and order_id_int in order_waiter:
                try:
