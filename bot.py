...
# ─── HTTP SERVER pour Render health check + webhook + rapports ───
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/daily":
            self._run_async(self._send_daily())
        elif self.path == "/monthly":
            self._run_async(self._send_monthly())
        else:
            pass
    ...
    def do_POST(self):
        ...
        self._run_async(application.process_update(update))
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b"ok")
    ...
    async def _send_daily(self):
        ...
        await application.bot.send_message(chat_id=DIRECTION_CHAT_ID, text=text, parse_mode="Markdown")
